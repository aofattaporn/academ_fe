...
const SideBar1 = () => {
  ...
  return (
    ...
    >
      <div
        className={`absolute cursor-pointer -right-3 top-3 w-8 h-8 border-dark-purple rounded-md bg-primary flex justify-center items-center text-white font-bold ${
      ...
      <ul className="pt-6">
        {pageItem.map((menu, index) => (
          <Link key={index} to={menu.navigate} replace>
            <li
              key={index}
              className={`flex  rounded-md p-2 py-3 cursor-pointer hover:bg-light-white text-dark text-md items-center gap-x-2 ${
